...
export class EventService {
  async getAllEvents(): Promise<EventData[]> {
    const event = await db
      .selectFrom("Events")
      .innerJoin("Booking_Halls", "Events.hall_id", "Booking_Halls.hall_id")
      .select([
        "Events.event_id",
        "Events.event_name",
        "Events.event_date",
  ...
  async getEventSeats(eventId: number) {
    const seats = await db
      ...
      .innerJoin("Seats", "Event_Seats.seat_id", "Seats.seat_id")
      .select([
  ...
  async getSingleEventSeat(eventId: number, eventSeatId: number) {
    return await db
      .selectFrom("Event_Seats")
      .selectAll()
